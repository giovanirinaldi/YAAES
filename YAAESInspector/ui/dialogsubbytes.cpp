...
#include "ui_dialogsubbytes.h"


#include "src/global.h"

DialogSubBytes::DialogSubBytes(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DialogSubBytes)
{
    ui->setupUi(this);
...
DialogSubBytes::~DialogSubBytes()
{
    delete[] result;
    delete ui;
}

void DialogSubBytes::setStateMatrix(unsigned char* state){
    this->state = state;
}

void DialogSubBytes::hexToUpperCaseText(unsigned char &hex, char* temp_string){
        snprintf(temp_string, 3, "%.2x", hex);
        temp_string[0] = toupper(temp_string[0]);
        temp_string[1] = toupper(temp_string[1]);
}

void DialogSubBytes::updateMatrix(){
